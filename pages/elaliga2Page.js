...
export async function getServerSideProps() {
  const programas = await Get("PROGRAMA", "ELALIGA2").getServerSideProps();
  const fechas = await GetFechas("ELALIGA2").getServerSideProps();
  return {
    props: {
...
export default function elaliga2({ datos, fechas }) {
  return (
    <div>
      <main>
        <Navbar />
