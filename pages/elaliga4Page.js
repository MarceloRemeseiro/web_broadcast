...
export async function getServerSideProps() {
  const programas = await Get("PROGRAMA", "ELALIGA4").getServerSideProps();
  const fechas = await GetFechas("ELALIGA4").getServerSideProps();
  return {
    props: {
...
export default function elaliga4({ datos, fechas }) {
  return (
    <div>
      <main>
        <Navbar />
