...
export async function getServerSideProps() {
  const programas = await Get("PROGRAMA", "ELALIGA1").getServerSideProps();
  const fechas = await GetFechas("ELALIGA1").getServerSideProps();
  return {
    props: {
...
export default function elaliga1({ datos, fechas }) {
  return (
    <div>
      <main>
        <Navbar />
